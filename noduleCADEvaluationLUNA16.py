...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    print(frvvlu/7)
    print(frvvlu/7, nxth)
    print(ssss[0], ssss[1], ssss[2], ssss[3], ssss[4], ssss[5], ssss[6])
    if performBootstrapping:
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        if performBootstrapping:
            ...
            np.save(os.path.join(outputDir,'%s_fps.npy' % (CADSystemName)), fps_bs_itp)
            np.save(os.path.join(outputDir,'%s_sens.npy' % (CADSystemName)), sens_bs_mean)
